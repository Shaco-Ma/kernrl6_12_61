...
static int __init its_of_probe(struct device_node *node)
{
	...
	for (np = of_find_matching_node(node, its_device_id); np;
		...
		//printk(KERN_ERR "%d--node name:%s--%s\n", __LINE__, np->name, np->full_name);
		if ((err = of_address_to_resource(np, 0, &res)) != 0) {
			pr_warn("%pOF: no regs?----%d\n", np, err);
			continue;
		}
		    ...
		    struct irq_domain *parent_domain, u8 irq_prio)
{
	...
	if (of_node)
	{
		//printk(KERN_ERR "of node name:%s----%s\n", of_node->name, of_node->full_name);
